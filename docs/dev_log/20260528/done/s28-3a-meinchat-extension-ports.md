...
**Status:** ✅ IMPLEMENTED & GREEN — 2026-05-29. Backend complete: extensibility
package (registry + 5 ports + defaults), `message`/`conversation` e2e columns +
constraints, `send_text` routed through `IBodyCodec`/`IPostSendHook`,
`GET /messaging/capabilities[?me=true]`, conversation protocol negotiation
(409/400 contract), Alembic migration `20260528_1100_meinchat_e2e`. **241
meinchat specs pass / 5 skipped; black + flake8 clean.** The existing meinchat
integration suite (no downstream plugin loaded) + a static no-downstream-imports
guard serve as the "plugin-free still works" oracle. NOTE: the migration
up/down/up matrix oracle is a CI job, not run locally — verify in CI.
**Revised 2026-05-28** to absorb the
critical review:
