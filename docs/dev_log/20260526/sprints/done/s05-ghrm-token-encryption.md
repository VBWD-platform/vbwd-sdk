...
**Done.** 9 new cipher/EncryptedString tests + 74 existing GHRM tests
all green; `pre-commit-check.sh --quick` (lint + 1973 unit) green.

### Implementation deviated from the original plan (§8 / cleaner design)

Sprint planned **service-layer encrypt/decrypt** wrapping every
read/write. Implemented as a **SQLAlchemy `TypeDecorator`
(`EncryptedString`)** instead — transparent at the ORM layer, so:

- GHRM service code change = **delete two TODO comments**. No new
  `cipher.encrypt(...)` calls, no test changes for the service.
- Future plugins reuse the column type by importing one symbol.
- §3-S SRP: encryption is a storage concern, kept at the storage layer.
- §5 DRY: one home for the cipher mechanics.

### Files

**New:**
- `vbwd-backend/vbwd/utils/crypto.py` — `TokenCipher` (Fernet) +
  `EncryptedString` (`TypeDecorator`) + `get_default_cipher` resolver
  with prod env / dev fallback chain + `reset_default_cipher_cache`
  test hook (~110 LOC).
- `vbwd-backend/tests/unit/test_token_cipher.py` — 9 contract tests
  covering round-trip, wrong-key, env-var resolution, FLASK_SECRET_KEY
  fallback, pristine-env fallback, `EncryptedString` round-trip, None
  handling, legacy-plaintext pass-through, the TODO-grep oracle.

**Modified:**
- `plugins/ghrm/src/models/ghrm_user_github_access.py` — `oauth_token`
  and `deploy_token` columns now use `EncryptedString` (was `db.Text`).
- `plugins/ghrm/src/services/github_access_service.py` — removed both
  `# TODO: encrypt` comments (encryption now happens transparently at
  the ORM boundary).
- `docker-compose.server.yaml` — added required `VBWD_TOKEN_ENCRYPTION_KEY`
  env var (`${VAR:?...}` form, helpful error message).
- `Makefile.server::secrets` — generator line for the encryption key.
- `tests/unit/test_secret_hygiene.py` — extended `PROD_SECRET_VARS` to
  include `VBWD_TOKEN_ENCRYPTION_KEY`.

### Acceptance verified

- 9 cipher tests green (round-trip, wrong-key raises `InvalidToken`,
  env-var precedence chain, ORM-layer round-trip, None handling, legacy
  plaintext warning, TODO grep is empty).
- All 74 GHRM unit tests still pass — proves the migration to the
  `TypeDecorator` is transparent to service code.
- `bin/pre-commit-check.sh --lint` green (Black + Flake8 + Mypy).

### Deferred / operator runbook needed

- **One-shot rotation script** for existing plaintext tokens in prod
  (if any). Today's `EncryptedString` returns legacy plaintext as-is
  with a warning log so operators can re-encrypt without data loss.
  Standalone script at `bin/rotate_ghrm_tokens.py` to be added when
  GHRM is actually enabled in prod — out of scope here.
- **Schema migration that adds the encryption** isn't needed (stored
  type was already `Text`); only the *value* shape changes from
  plaintext to Fernet-base64.
- The `EncryptedString` length parameter (originally planned) was
  dropped — speculative, no caller needs it, mypy complained about the
  `impl` reassignment. §8 no overengineering.
