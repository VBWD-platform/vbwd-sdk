...
**Done — 189 payment-plugin tests green.**

**Important finding (not in the sprint plan):** the payment-plugin
models can NOT inherit `BaseModel` because they differ in two ways:
1. They use ``DateTime(timezone=True)`` (TZ-aware), but `BaseModel`
   uses naive ``DateTime`` with `utcnow` helper from
   `vbwd.utils.datetime_utils`.
2. They use Postgres-side UUID generation
   (`server_default=db.text("gen_random_uuid()")`), but `BaseModel`
   uses Python-side `default=uuid4`.

Either change would alter behaviour. The cleanest dedup that doesn't
break anything: extract a **mixin** for just the timestamps.

**Patches:**
- `vbwd/models/base.py` — new `TzAwareTimestampMixin` with TZ-aware
  `created_at` / `updated_at` plus the shared `_utcnow_aware()` helper.
  Documented why it co-exists with `BaseModel`.
- 6 payment plugin models updated to inherit
  `(TzAwareTimestampMixin, db.Model)`:
  - `plugins/conekta/conekta/models.py`
  - `plugins/mercado_pago/mercado_pago/models.py`
  - `plugins/truemoney/truemoney/models.py`
  - `plugins/toss_payments/toss_payments/models.py` (TossPayment only;
    TossCashReceipt has its own non-timestamp `issued_at` column —
    kept inline)
  - `plugins/c2p2/c2p2/models.py`
  - `plugins/promptpay/promptpay/models.py`

**LOC removed:** ~10 lines per model × 6 = ~60 LOC of pure duplication
gone. Future timestamp-policy changes (e.g. switching all to
`func.now()` server-side) happen in one place.

**Acceptance verified:**
- 189 existing payment plugin tests still green (no behaviour change).
- `to_dict()` output unchanged (the timestamp columns are emitted the
  same way — Python-side ISO formatting per existing model logic).

**Deferred / acknowledged residual:** `to_dict()` is still
re-implemented per plugin (each plugin's `to_dict` calls `isoformat()`
on its own columns). Extracting that would require a shared base that
knows the full column list — out of scope for this slice (§8 narrow).
