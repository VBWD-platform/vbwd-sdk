...
**Done.** New parametrised test (25 plugin names × 1) green;
S01 oracle still green.

**Patch:** `vbwd/app.py:206-231` rewritten:
- Default-enable logic now reads from `plugins/plugins.json.dist` — any
  plugin can be a fresh-install default just by editing that file.
- Blueprint registration is a single loop that calls both
  `get_blueprint()` and `get_admin_blueprint()` on every plugin; the
  `BasePlugin.get_admin_blueprint() = None` default means plugins
  without an admin surface no-op silently.
- Removed the analytics-specific branch entirely (was 2 hardcoded
  string literals at lines 211 + 219).

**Acceptance verified:**
- All 25 parametrised plugin-name guards green (zero hardcoded names).
- S01 import oracle still green.

**Deferred:** none. Self-contained.
