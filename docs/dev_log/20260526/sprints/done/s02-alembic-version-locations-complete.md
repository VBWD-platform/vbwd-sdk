...
**Done.** New test `tests/unit/test_alembic_config.py` (2 cases) green.

**Patches:**
- `alembic.ini:36` — appended the 6 missing plugin migration dirs
  (`c2p2`, `conekta`, `mercado_pago`, `promptpay`, `toss_payments`,
  `truemoney`). Did NOT switch to `recursive_version_locations = true`
  — the explicit list is greppable and grows by one line per plugin;
  recursive scan would also pull in `__pycache__` dirs (§8 keep it boring).
- `Makefile.server:67` — `alembic upgrade head` → `alembic upgrade heads`.

**Acceptance verified:**
- `test_every_plugin_with_migrations_is_registered_in_alembic_ini` green.
- `test_makefile_server_uses_alembic_upgrade_heads_plural` green.
- All 15 plugin migration dirs now discoverable; data-integrity bomb defused.

**Deferred (out of S02 scope):**
- CI fail-fast on migration step → covered by [[s15]] (with the plugin-clone fail-fast).
- Decoupling migration from gunicorn `CMD` → [[s03]].
