...
**Done.** New test `tests/unit/test_secret_hygiene.py` (2 cases) green.

**Investigation finding:** `.env` is **already untracked** (confirmed
with `git ls-files .env` → empty). The git-hygiene side of the sprint
was a no-op; only the compose-form fix was needed.

**Patches:**
- `docker-compose.server.yaml` — three secrets (`VBWD_FLASK_SECRET`,
  `VBWD_JWT_SECRET`, `VBWD_DB_PASSWORD`) switched from bare `${VAR}`
  (silently empty) to required form
  `${VAR:?VAR must be set (see: make secrets)}`. Applied to both the
  backend service env and the new `vbwd_migrate` service from S03, plus
  the postgres `POSTGRES_PASSWORD`.
- `Makefile.server` — new `make secrets` target that prints `export …`
  lines with cryptographically-strong defaults
  (`secrets.token_hex(32)` / `secrets.token_urlsafe(24)`).

**Acceptance verified:**
- `test_server_compose_secrets_use_required_form` green.
- `test_env_file_listed_in_gitignore` green.
- `docker compose -f docker-compose.server.yaml config` with all three
  env vars unset would now abort with the helpful error string instead
  of producing a config with empty secrets.

**Deferred / non-applicable to S04:**
- Dev compose (`docker-compose.yaml`) intentionally keeps `${VAR:-default}`
  form — devs need first-run to work without manual setup, and the
  defaults are known-public placeholders explicitly labelled
  `…-change-in-production`. The defence-in-depth is "prod compose is
  the gate", not "dev compose can't have defaults".
- Secret rotation playbook (rotating the committed-but-public dev
  placeholder values) — separate runbook PR; the placeholders never
  ran in prod.
