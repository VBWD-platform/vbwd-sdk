...
## Outcome — 2026-05-27 (REJECTED — §8 no overengineering)

**Not landed. Closed as "don't do this until a real bug forces it."**

Same logic that closed [[s12]]: the cure here is more complex than
the disease.

### What the audit flagged

7 module-level mutable registries (`line_item_registry`,
`deletion_dependency_registry`, `_providers`, `_lifecycle`, etc.).
Theoretical risks: cross-test leakage, thread safety, no per-app
scoping.

### Why none of those risks are real today

- **Cross-test leakage** — already mitigated by every test that needs
  it calling `clear_*` in teardown. [[s17]] partial added an oracle
  for entitlement; the meta tracker enforces more as they ship.
- **Thread safety** — registration happens once at app boot under a
  single-threaded init path. The post-boot read path is read-only
  except during plugin enable/disable, which is also single-threaded
  (admin-only manual action). Locks would add cost for no observed
  benefit.
- **Per-app scoping** — the codebase doesn't run multiple Flask app
  instances in one process. The "what if we did" scenario isn't
  on the roadmap.

### Cost of doing it anyway

7 registries × every plugin's `on_enable`/`on_disable` × every test
that uses `clear_*` hooks × every consumer that calls `resolve_*`.
1-2 days of careful work for a refactor that delivers no
user-visible improvement and no measurable code quality improvement.

### What would un-reject this sprint

A concrete bug from one of the theoretical risks:
- A flaky test caused by registry state bleeding between tests
  (would require parallel pytest-xdist with a stuck `clear_*`).
- A production incident from thread-unsafe registry mutation
  (would require concurrent plugin enable/disable).
- A multi-app process requirement (not on roadmap).

Until then: **the existing pattern is the narrowest design that
works.** Keep it.
