...
**Not landed.** On close inspection the existing design is already
ISP-acceptable:

- The "optional" methods (`resolve_catalog_item_id`,
  `is_recurring_line_item`, `recurring_billing_spec`) have **no-op
  defaults** on the base `ILineItemHandler`. Handlers that don't care
  about them inherit `None` / `False` for free — no stub work.
- The registry can call those methods on every handler without
  `isinstance(handler, Mixin)` plumbing.
- Splitting into Protocols would add: (a) two new interface symbols,
  (b) runtime `isinstance` checks at every dispatch site, (c) more
  imports in every plugin. Net change for callers: ~zero. Net change
  for the codebase: more code.

The original audit flagged this as a "design smell, not a bug" — and
the cure here is genuinely worse than the disease for today's two
optional concerns. **The right test for this sprint's value is "would
splitting eliminate ≥1 line of stub code in any handler today?"** The
answer is no. Revisit IF a 3rd optional concern appears or a real
handler trips over the inheritance.

**§8 no-overengineering: don't do this refactor.**
